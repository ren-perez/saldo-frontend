...
export const cloneUserData = mutation({
  ...
  handler: async (ctx, { fromUserId, toUserId }) => {
    ...
    // --- 1. Category Groups ---
    const groups = await ctx.db.query("category_groups").filter(q => q.eq(q.field("userId"), fromUserId)).collect();
    ...
    // --- 2. Categories ---
    const categories = await ctx.db.query("categories").filter(q => q.eq(q.field("userId"), fromUserId)).collect();
    for (const c of categories) {
      const { _id, userId, groupId, ...rest } = c;
    ...
    // --- 3. Accounts ---
    const accounts = await ctx.db.query("accounts").filter(q => q.eq(q.field("userId"), fromUserId)).collect();
    ...
    // --- 4. Goals ---
    const goals = await ctx.db.query("goals").filter(q => q.eq(q.field("userId"), fromUserId)).collect();
    for (const g of goals) {
      const { _id, userId, linked_account_id, ...rest } = g;
    ...
    // --- 5. Transactions ---
    const txs = await ctx.db.query("transactions").filter(q => q.eq(q.field("userId"), fromUserId)).collect();
    for (const t of txs) {
      const { _id, userId, accountId, categoryId, ...rest } = t;
    ...
    const contribs = await ctx.db.query("goal_contributions").filter(q => q.eq(q.field("userId"), fromUserId)).collect();
    for (const c of contribs) {
      const { _id, userId, goalId, ...rest } = c;
    ...
    const gplans = await ctx.db.query("goal_monthly_plans").filter(q => q.eq(q.field("userId"), fromUserId)).collect();
    for (const g of gplans) {
      const { _id, userId, goalId, ...rest } = g;
    ...
    // --- 8. Presets ---
    const presets = await ctx.db.query("presets").filter(q => q.eq(q.field("userId"), fromUserId)).collect();
    const presetIdMap = new Map();
    for (const p of presets) {
      const { _id, userId, ...rest } = p;
    ...
    const ig = await ctx.db.query("ignored_transfer_pairs").filter(q => q.eq(q.field("userId"), fromUserId)).collect();
    for (const i of ig) {
      const { _id, userId, outgoingTransactionId, incomingTransactionId, ...rest } = i;
      // ⚠️ This assumes we copied transactions first.
      await ctx.db.insert("ignored_transfer_pairs", {
        ...rest,
        userId: toUserId,
        outgoingTransactionId, // ideally map to new txId if you build txIdMap
